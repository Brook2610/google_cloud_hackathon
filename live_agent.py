...
OPTIMIZED: Matches the smooth live/web_voice implementation pattern.
"""
import asyncio
import json
import os
...
Your goal is to gather website requirements by asking MAX 2 questions.

Question flow:
1. Greet warmly (ሰላም!) and ask what type of website they want with what featuers.
2. Ask about colors/style preferences ( colorful, minimal, etc.)

Rules:
- Keep questions SHORT and clear
- Listen carefully to their responses
- After 2 questions (or earlier if you have enough info), call the submit_website_description tool
...
def get_live_config(business_name: Optional[str] = None, place_id: Optional[str] = None) -> types.LiveConnectConfig:
    """Create the LiveConnectConfig matching web_voice's smooth settings."""
    ...
    return types.LiveConnectConfig(
        ...
        system_instruction=types.Content(
            parts=[types.Part.from_text(text=get_system_prompt(business_name, place_id))],
            role="system",
        ),
    )
...
class LiveAssistant:
    ...
    OPTIMIZED: Uses direct streaming pattern like web_voice for smooth audio.
    """
    ...
    async def start(self):
        ...
        config = get_live_config(self.business_name, self.place_id)
    ...
    async def send_audio(self, audio_data: bytes):
        """Send audio data directly to Gemini - no queue for lower latency."""
        if not self._running or not self.session or not audio_data:
            return
    ...
    async def _receive_loop(self):
        """Receive and process responses from Gemini - optimized for smooth audio."""
        try:
            while self._running:
                ...
                async for response in turn:
                    ...
                    # Extract audio chunks efficiently
                    audio_chunks = []
                    ...
                    # Send audio immediately - no batching
                    ...
                    if sc and getattr(sc, "output_transcription", None):
                        ...
                        if getattr(transcript, "text", None) and self.on_transcript:
            ...
            if getattr(tc, "function_calls", None):
                for fc in tc.function_calls:
                    ...
                    if fname == "submit_website_description":
                        ...
                        # Give time for final audio then stop
